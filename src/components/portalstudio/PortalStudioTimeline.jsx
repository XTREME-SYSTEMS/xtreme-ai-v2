          ...
          return (
            <div key={step.to} className="flex items-center" data-step-idx={i}>
              ...
              >
                ...
                <span className={cn("hidden whitespace-nowrap text-[11px] font-medium sm:block", isCurrent ? "text-lime-400" : "text-white/30")}>
                  {step.label}
                </span>
              </button>
              {i < visibleSteps.length - 1 && (
                <div className={cn("mx-0.5 h-0.5 w-3 shrink-0 rounded-full sm:mx-1 sm:w-5", i < currentIdx ? "bg-lime-400" : "bg-white/10")} />
              )}
            </div>
          );
        })}
