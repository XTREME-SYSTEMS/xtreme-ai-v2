        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-4">
          {TIERS.map((t, i) => (
            <motion.div key={t.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.08 }}
              ...
              {t.free ? (
                <Link to="/dashboard" className="mt-5 inline-flex items-center justify-center gap-1.5 rounded-xl border border-white/20 px-4 py-3 text-sm font-bold text-white transition-all hover:bg-white hover:text-black">
                  {t.cta} <ArrowRight className="h-4 w-4" />
                </Link>
              ) : t.contact ? (
                <a href="/#contact" className="mt-5 inline-flex items-center justify-center gap-1.5 rounded-xl border border-white/20 px-4 py-3 text-sm font-bold text-white transition-all hover:bg-white hover:text-black">
                  {t.cta} <ArrowRight className="h-4 w-4" />
                </a>
              ) : (
                <button onClick={() => handleCheckout(annual ? t.annualId : t.monthlyId, t.name)} disabled={loading === t.name}
                  className={`mt-5 inline-flex items-center justify-center gap-1.5 rounded-xl px-4 py-3 text-sm font-bold transition-all disabled:opacity-50 ${t.highlight ? "bg-lime-400 text-black hover:bg-lime-300" : "border border-white/20 text-white hover:bg-white hover:text-black"}`}>
