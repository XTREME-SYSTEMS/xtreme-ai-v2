      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        {showHeader && (
          ...
          className="mx-auto max-w-2xl text-center">
            <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-lime-400/15 px-3 py-1 text-xs font-bold uppercase tracking-wider text-lime-600">Pricing</div>
            ...
            <div className="mt-7 inline-flex items-center gap-3 rounded-full border border-black/15 bg-zinc-100 p-1">
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-4">
          {TIERS.map((t, i) => (
            <motion.div key={t.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.08 }}
              className={`relative flex flex-col rounded-2xl border p-6 ${t.highlight ? "border-lime-400 bg-lime-400/5 lg:scale-105" : "border-black/10 bg-white"}`}>
              ...
              {annual && t.yearly ? <div className="mt-1 text-xs text-lime-600">Billed ${t.yearly.toLocaleString()}/yr</div> : <div className="mt-1 text-xs text-black/20">&nbsp;</div>}
