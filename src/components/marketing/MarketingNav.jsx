      ...
      <nav className={cn("w-full bg-white transition-all duration-300", scrolled ? "border-b border-black/10 shadow-sm" : "border-b border-black/5")}>
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          ...
          <div className="hidden items-center gap-3 md:flex">
            <Link to="/login" className="text-sm font-medium text-black/70 hover:text-amber-600">Login</Link>
            <Link to="/pricing" className="rounded-lg bg-amber-400 px-4 py-2 text-sm font-semibold text-black transition-all hover:bg-amber-300 hover:shadow-lg hover:shadow-amber-400/40">Get Started</Link>
          </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-white md:hidden">
          ...
          <div className="flex flex-col gap-1 px-4 py-6">
            ...
            <Link to="/login" onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 text-base font-medium text-black/80 hover:bg-amber-400/10">Login</Link>
