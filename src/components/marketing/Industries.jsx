    ...
    <section id="solutions" className="bg-white py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="text-center">
          <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-lime-400/15 px-3 py-1 text-xs font-bold uppercase tracking-wider text-lime-600">Solutions</div>
          <h2 className="text-3xl font-bold tracking-tight text-black sm:text-4xl lg:text-5xl">Built for local service businesses</h2>
          <p className="mt-4 text-lg text-black/60">From the first click to the fifth repeat sale, Lead Generation Near Me is tuned to the way local service businesses win work.</p>
        </div>
        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {INDUSTRIES.map((ind, i) => (
            <motion.div key={ind.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }}
              className="rounded-2xl border border-black/10 bg-zinc-50 p-6 transition-all hover:border-lime-400 hover:bg-white hover:shadow-lg">
        ...
        <div className="mt-16 grid grid-cols-1 gap-6 sm:grid-cols-3">
          {STATS.map((s) => (
            <div key={s.label} className="text-center">
              <div className="text-4xl font-bold text-lime-600 sm:text-5xl">{s.value}</div>
