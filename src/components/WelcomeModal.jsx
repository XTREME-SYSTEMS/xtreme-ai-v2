...
import { Rocket, MapPin, Sparkles, ArrowRight, CheckCircle2, X, Compass, Wand2, GitBranch } from "lucide-react";
...
export default function WelcomeModal({ user, role }) {
  ...
  const start = isAdmin
    ? [
        ...
        { to: "/brand-factory", icon: Wand2, label: "Brand Factory", desc: "AI concept-to-reality brand pipeline (logo, card, brochure, social, video)." },
        { to: "/clone-pipeline", icon: GitBranch, label: "Clone Pipeline", desc: "Clone → rebrand → deploy a site end-to-end." },
      ]
    : [
        { to: "/dashboard", icon: Sparkles, label: "AI Onboarding", desc: "Answer a few questions so your team knows your business." },
        { to: "/brand-factory", icon: Wand2, label: "Brand Factory", desc: "Build your full brand kit step-by-step with AI." },
      ];
  ...
  const steps = isAdmin
    ...
    : [
        ...
        "Then open Brand Factory and let the AI interview you about your business.",
        "Step through Logo → Business Card → Brochure → Social → Video.",
        "Approve any work your team sends your way in the Pending Approvals panel.",
      ];

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={close}>
      <div
        className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-lime-400/30 bg-zinc-900 shadow-2xl"
      ...
      >
        ...
        <div className="bg-gradient-to-br from-lime-400/15 to-transparent p-6 pb-4">
          <div className="flex items-center gap-2 text-lime-400">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider">Lead Gen Near You</span>
          </div>
          <h2 className="mt-2 text-2xl font-bold text-white">
            Hello{user?.full_name ? `, ${user.full_name}` : ""}! 👋
          </h2>
          <p className="mt-1 text-sm text-white/60">
            Welcome aboard. Here's a quick tour to get you moving.
          </p>
        </div>
