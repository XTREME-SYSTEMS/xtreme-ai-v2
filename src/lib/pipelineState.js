...
// signals: optional { indexRankComplete, optimizeComplete } from getPipelineSignals
export function computePipelineState(user, approvals = [], signals = {}) {
  ...
  return UNIVERSAL_PIPELINE.map((step) => {
    ...
    if (step.key === "onboarding") {
      ...
      completed = !!signals.optimizeComplete;
    } else if (step.gate) {
...
export function currentPipelineStep(user, approvals = [], signals = {}) {
...
export function pipelineProgress(user, approvals = [], signals = {}) {
  const states = computePipelineState(user, approvals, signals);
