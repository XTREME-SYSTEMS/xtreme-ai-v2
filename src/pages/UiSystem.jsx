...
export default function UiSystem() {
  ...
  const generate = async () => {
    ...
    setError("");
    try {
      const res = await base44.functions.invoke("generateUiSystem", {
        architecture,
        productType: build.product_type,
        businessName: build.business_name,
      });
      const spec = res?.data?.data || res?.data;
      if (!spec) throw new Error("No UI system returned");
      await autoBuild.saveBuild({
        ui_system: spec,
        current_step: "ui_system",
        logs: [...(build.logs || []), `[${new Date().toISOString()}] UI system generated`],
      });
    } catch (e) {
      setError(e?.message || "Couldn't generate UI system. Try again.");
    } finally {
      setGenerating(false);
    }
  };
  ...
  const approve = async () => {
    ...
    if (!visited.includes("/ui-system")) visited.push("/ui-system");
    await autoBuild.saveBuild({
      visited_steps: visited,
      logs: [...(build.logs || []), `[${new Date().toISOString()}] UI system approved`],
    });
    ...
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      {generating && (
        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-white/10 bg-zinc-950 py-16">
          ...
          <p className="text-xs text-white/30">This takes 15-30 seconds. The AI is creating a complete design system for your product.</p>
        </div>
      )}
