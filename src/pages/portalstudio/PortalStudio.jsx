...
export default function PortalStudio() {
  ...
  if (!isActive) {
    return (
      <div className="mx-auto max-w-4xl space-y-8">
        <section className="rounded-xl border border-yellow-300/40 bg-yellow-300/5 p-6">
        ...
        <section>
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-yellow-300">Build Projects</h2>
            <button
              onClick={() => setShowNew((v) => !v)}
              className="inline-flex items-center gap-1.5 rounded-lg bg-yellow-300 px-3 py-1.5 text-xs font-semibold text-black hover:bg-yellow-300"
            >
          ...
          {showNew && (
            <div className="mt-4 flex gap-2 rounded-xl border border-white/10 bg-zinc-950 p-4">
              <input
                ...
                className="flex-1 rounded-lg border border-white/15 bg-black px-3 py-2 text-sm text-white placeholder-white/30 focus:border-yellow-300 focus:outline-none"
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-yellow-300 px-4 py-2 text-sm font-semibold text-black hover:bg-yellow-300 disabled:opacity-50"
              >
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {projects.map((p) => (
                <button
                  ...
                  className="group flex items-center gap-3 rounded-xl border border-white/10 bg-zinc-950 p-4 text-left transition-colors hover:border-yellow-300/50"
                >
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-yellow-300/10">
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-10">
      ...
      <section>
        <h2 className="text-xs font-semibold uppercase tracking-wider text-yellow-300">Your build pipeline</h2>
        <p className="mt-2 text-sm text-white/70">
          Every build follows the same steps. Steps marked <span className="font-semibold text-yellow-300">Approval</span> pause for sign-off in the real portal — here they auto-complete so you can move fast.
        </p>
          ...
          {states.map((s, i) => {
            ...
            return (
              <StepTag
                key={step.key}
                {...stepProps}
                className={`flex gap-4 rounded-xl border border-white/10 bg-zinc-950 p-4 ${step.to ? "transition-colors hover:border-yellow-300/40" : ""}`}
              >
                <div className="flex flex-col items-center">
                  <div className={`flex h-10 w-10 items-center justify-center rounded-full border text-sm font-bold ${
                    ...
                    {s.completed ? <CheckCircle2 className="h-5 w-5 text-yellow-300" /> : i + 1}
                  </div>
                ...
                <div className="flex-1 pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    {s.completed && (
                      <span className="ml-auto inline-flex items-center gap-1 text-[10px] font-medium text-yellow-300">
