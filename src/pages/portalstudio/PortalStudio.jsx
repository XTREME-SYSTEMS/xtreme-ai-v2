...
export default function PortalStudio() {
  ...
  useEffect(() => { document.title = "Portal Studio · Lead Gen Near You"; }, []);
  ...
  const createProject = async () => {
    ...
    try {
      const created = await base44.entities.PortalStudioProject.create({
        ...
        logs: [`[Studio] Project created: ${newName.trim()}`],
      });
      ...
      <div className="mx-auto max-w-4xl space-y-8">
        <section className="rounded-xl border border-lime-400/40 bg-lime-400/5 p-6">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-lime-400">
            <Hammer className="h-4 w-4" /> Portal Studio
          </div>
          <h1 className="mt-3 text-2xl font-semibold text-white sm:text-3xl">Clone · Refine · Harden · Brand</h1>
          <p className="mt-2 text-sm text-white/70">
            An isolated copy of the client portal. Pick a sandbox project to walk the full build journey, or create a new one. Nothing here touches real client data.
          </p>
        </section>

        <section>
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-lime-400">Sandbox Projects</h2>
            <button
              onClick={() => setShowNew((v) => !v)}
              className="inline-flex items-center gap-1.5 rounded-lg bg-lime-400 px-3 py-1.5 text-xs font-semibold text-black hover:bg-lime-300"
            >
          ...
          {loading ? (
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-zinc-950 p-8 text-center">
              ...
              <p className="mt-3 text-sm text-white/50">No sandbox projects yet. Create one to start refining the portal.</p>
            </div>
          ) : (
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {projects.map((p) => (
                <button
                  ...
                  className="group flex items-center gap-3 rounded-xl border border-white/10 bg-zinc-950 p-4 text-left transition-colors hover:border-lime-400/50"
                >
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-10">
      <section className="rounded-xl border border-lime-400/40 bg-lime-400/5 p-6">
        ...
        <h1 className="mt-3 text-2xl font-semibold text-white sm:text-3xl">Portal Studio — Sandbox Build</h1>
        <p className="mt-2 text-sm text-white/70">
          This is the exact client portal experience, running against your isolated sandbox project. Walk every step, refine the UX, harden the flows.
        </p>
      ...
      <section>
        ...
        <p className="mt-2 text-sm text-white/70">
          Every build follows the same steps. Steps marked <span className="font-semibold text-lime-400">Approval</span> pause for sign-off in the real portal — here they auto-complete so you can move fast.
        </p>
