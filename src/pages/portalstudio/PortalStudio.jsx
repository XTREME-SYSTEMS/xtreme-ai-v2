...
export default function PortalStudio() {
  ...
  if (!isActive) {
    return (
      <div className="mx-auto max-w-4xl space-y-8">
        <section className="rounded-xl border border-yellow-400/40 bg-yellow-400/5 p-6">
        ...
        <section>
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-yellow-400">Build Projects</h2>
            <button
              onClick={() => setShowNew((v) => !v)}
              className="inline-flex items-center gap-1.5 rounded-lg bg-yellow-400 px-3 py-1.5 text-xs font-semibold text-black hover:bg-yellow-300"
            >
          ...
          {showNew && (
            <div className="mt-4 flex gap-2 rounded-xl border border-white/10 bg-zinc-950 p-4">
              <input
                ...
                className="flex-1 rounded-lg border border-white/15 bg-black px-3 py-2 text-sm text-white placeholder-white/30 focus:border-yellow-400 focus:outline-none"
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-yellow-400 px-4 py-2 text-sm font-semibold text-black hover:bg-yellow-300 disabled:opacity-50"
              >
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {projects.map((p) => (
                <button
                  ...
                  className="group flex items-center gap-3 rounded-xl border border-white/10 bg-zinc-950 p-4 text-left transition-colors hover:border-yellow-400/50"
                >
                  ...
                  <ArrowRight className="h-4 w-4 shrink-0 text-white/30 group-hover:text-yellow-400" />
                  <button
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-10">
        ...
        <div className="mt-4 flex items-center gap-3">
          <div className="flex-1">
            ...
            <div className="mt-1 h-2 overflow-hidden rounded-full bg-white/10">
              <div className="h-full rounded-full bg-yellow-400 transition-all" style={{ width: `${progress.percent}%` }} />
            </div>
          </div>
          <button
      ...
      <section>
        <h2 className="text-xs font-semibold uppercase tracking-wider text-yellow-400">Your build pipeline</h2>
        <p className="mt-2 text-sm text-white/70">
          Every build follows the same steps. Steps marked <span className="font-semibold text-yellow-400">Approval</span> pause for sign-off in the real portal — here they auto-complete so you can move fast.
        </p>
          ...
          {states.map((s, i) => {
            ...
            return (
              <StepTag
                key={step.key}
                {...stepProps}
                className={`flex gap-4 rounded-xl border border-white/10 bg-zinc-950 p-4 ${step.to ? "transition-colors hover:border-yellow-400/40" : ""}`}
              >
                <div className="flex flex-col items-center">
                  <div className={`flex h-10 w-10 items-center justify-center rounded-full border text-sm font-bold ${
                    s.completed ? "border-yellow-400 bg-yellow-400/10 text-yellow-400" :
                    s.isCurrent ? "border-yellow-400 bg-yellow-400/15 text-yellow-400" :
                    "border-white/15 bg-zinc-900 text-white/80"
                  }`}>
                    {s.completed ? <CheckCircle2 className="h-5 w-5 text-yellow-400" /> : i + 1}
                  </div>
                ...
                <div className="flex-1 pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    {s.completed && (
                      <span className="ml-auto inline-flex items-center gap-1 text-[10px] font-medium text-yellow-400">
