...
// Top 10 ideas are ALWAYS visible at the top (regardless of filter).
// Below that, a filterable list of all ideas.
// Each idea has a Deploy button that provisions it to the Auto Builder.
export default function VisionCortex() {
  ...
  const runCycle = async () => {
    try {
      ...
      const discoverRes = await base44.functions.invoke("visionCortexDiscover", {
        max_sources: 6, max_ideas_per_source: 10, max_total: 50, triggered_by: "manual",
      });
      ...
      if (discoverData?.run_id) {
        await base44.functions.invoke("visionCortexValidate", {
          run_id: discoverData.run_id, top_n: 10, min_score: 40, generate_summaries: true,
        });
      }
      await load();
    } catch (e) {
  ...
  const provisionIdea = async (ideaId) => {
    try {
      setDeploying((p) => ({ ...p, [ideaId]: true }));
      const res = await base44.functions.invoke("visionCortexProvision", { idea_id: ideaId, auto_advance: true });
      ...
      setDeploying((p) => ({ ...p, [ideaId]: false }));
    }
  };

  const filteredIdeas = filter === "provisioned"
    ...
    <div className="mx-auto max-w-5xl space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-semibold text-white flex items-center gap-2">Vision Cortex</h1>
          <p className="text-sm text-white/50">
            AI-Assisted Idea Discovery & Validation Engine — feeds the Auto Builder
            {lastRun && <span className="ml-2 text-white/30">· Last cycle: {new Date(lastRun.created_date).toLocaleString()}</span>}
          </p>
        </div>
        <button onClick={runCycle} disabled={running} className="inline-flex items-center gap-1.5 rounded-lg bg-lime-400 px-3 py-2 text-xs font-medium text-black hover:bg-lime-300 disabled:opacity-50">
          {running ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : <Rocket className="h-3.5 w-3.5" />}
          {running ? "Cycling..." : "Run Full Cycle"}
        </button>
      </div>

      {error && <div className="rounded-xl border border-red-400/40 bg-red-400/5 p-3 text-sm text-red-300">{error}</div>}
      ...
      <div>
        <div className="mb-2 flex items-center gap-2">
          <h2 className="text-sm font-semibold text-white">Top 10 Ideas</h2>
          <span className="text-xs font-normal text-white/40">({topIdeas.length})</span>
          <span className="ml-2 rounded-full bg-lime-400/10 px-2 py-0.5 text-[9px] font-medium text-lime-400">Always visible</span>
        </div>

        {loading ? (
          <div className="flex justify-center py-10"><RefreshCw className="h-6 w-6 animate-spin text-lime-400" /></div>
        ) : topIdeas.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-zinc-950 py-12 text-center">
            ...
            <p className="mt-2 text-sm text-white/50">No top 10 ideas yet. Run a cycle to discover, score, and rank the top 10.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {topIdeas.map((idea) => (
              <VisionCortexIdeaRow
                ...
                onDeploy={provisionIdea}
                deploying={!!deploying[idea.id]}
              />
            ))}
          </div>
        )}
      ...
      {/* ALL IDEAS — filterable */}
      <div>
        <div className="mb-2 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-white">
            {filter === "autonomous" ? "Fully Autonomous Systems" : filter === "provisioned" ? "Provisioned Ideas" : "All Ideas"}
            <span className="ml-2 text-xs font-normal text-white/40">({filteredIdeas.length})</span>
          </h2>
          <div className="flex items-center gap-2">
            <Filter className="h-3.5 w-3.5 text-white/40" />
            {["all", "autonomous", "provisioned"].map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
              ...
              >
                {f === "all" ? "All" : f === "autonomous" ? "Autonomous Systems" : "Provisioned"}
              </button>
            ))}
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex justify-center py-6"><RefreshCw className="h-5 w-5 animate-spin text-white/30" /></div>
        ) : filteredIdeas.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-zinc-950 py-8 text-center">
            <p className="text-sm text-white/40">No ideas match this filter.</p>
          </div>
        ) : (
          <div className="space-y-2">
      ...
      <div className="rounded-xl border border-white/10 bg-zinc-950 p-4">
        <div className="flex items-center gap-2 text-xs text-white/50">
          ...
          <span>Vision Cortex runs automatically once every 24 hours via the "Vision Cortex Daily Cycle" workflow. Each cycle: scrape 6+ sources → extract 50 ideas → score with YC framework → select top 10 → generate exhaustive summaries.</span>
        </div>
      </div>

      <VisionCortexIdeaModal idea={selectedIdea} open={!!selectedIdea} onClose={() => setSelectedIdea(null)} onProvision={provisionIdea} />
    </div>
  );
}