...
import { Boxes, Loader2, RefreshCw, AlertCircle, Check, Package, Wrench, Image, Video, Palette, Eye } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="rounded-xl border border-lime-400/40 bg-lime-400/5 p-5 sm:p-6">
        ...
        <h1 className="mt-2 text-xl font-semibold text-white sm:text-2xl">Real XPS Products, Equipment & Marketing</h1>
        <p className="mt-1 text-sm text-white/60">
          Scraped from xtremepolishingsystems.com and their Instagram/YouTube. The auto builder injects these real
          assets into every flooring-industry build instead of generic placeholders.
        </p>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          >
            {ingesting ? <><Loader2 className="h-4 w-4 animate-spin" /> Ingesting from XPS…</> : <><RefreshCw className="h-4 w-4" /> Ingest Catalog</>}
          </button>
          <button
            onClick={loadAssets}
            disabled={loading}
          ...
          <div className="mt-3 flex items-start gap-2 rounded-lg border border-lime-400/50 bg-lime-400/10 px-3 py-2.5 text-sm text-lime-300">
            ...
            <div>
              ...
              <div className="mt-1 text-xs text-lime-300/70">
                {ingestResult.breakdown?.products || 0} products · {ingestResult.breakdown?.equipment || 0} equipment · {ingestResult.breakdown?.socialPosts || 0} social posts · {ingestResult.breakdown?.trainingVideos || 0} training videos · {ingestResult.breakdown?.brandAssets || 0} brand assets
              </div>
            </div>
          </div>
        )}
      ...
      <div className="mt-4">
        {loading ? (
          ...
          <div className="rounded-xl border border-white/10 bg-zinc-950 py-16 text-center">
            ...
            <p className="mt-3 text-sm text-white/50">No {activeCategory} assets yet. Run ingestion to populate.</p>
          </div>
        ) : (
